...
# 端云协同瘦客户端架构与服务端 BFF 规范

> **适用场景与目标**：本规范定义了 FoloToy AI Passport（ESP32-C3）作为**端云协同瘦客户端（Thin Client）**连接自建私有服务器或云端 BFF 的软硬件交互标准。适用于网络语音对讲、AI 对话卡片、流式音视频、大模型交互、远程低代码联动等联网场景。
>
> *(注：板级硬件引脚、总线与电气参数遵循官方权威源头：`components/bsp/include/bsp_pins.h` 与 `docs/hardware-design/AI_HARDWARE_DEVELOPMENT_GUIDE.md`。)*

---

## 1. 核心架构范式：物理终端（Client）+ 云端中枢（BFF / Brain）

系统采用端云解耦架构：ESP32-C3 设备作为物理交互界面，处理传感器感知、屏幕渲染与音频输入输出；私有服务器或 PC 承担复杂计算、大模型调用与业务状态存储：
...
```text
  ...
  │    • 嵌入式字库 (Montserrat 英文 + 独立 Flash 分区中的 837 KB CJK 常用字库)│
...
> [!WARNING]
> **红线警示**：REST API 仅允许用于设备初次上线激活、配网校验等低频无状态流程。**严禁**将 REST API 用于语音上传、实时问答、大模型 Token 吐字流、长文本翻页或卡片状态同步。
...
所有高实时、流式交互复用这单一 WebSocket 管道。板载仅有 `UP`、`DOWN`、`OK` 三个物理按键（共用 GPIO0 ADC0 电阻分压梯），PTT 对讲录音通常映射为**长按/按住 `OK` 键**，`UP`/`DOWN` 键用于视口分页滚动。

数据流通过**区分文本帧与二进制帧**来切分：

#### 1. 上行数据（客户端 ──► 服务端）
| 帧类型 | 内容与格式 | 传输规格与业务语义 |
| :--- | :--- | :--- |
| **二进制帧** | 麦克风录音 PCM 数据流 | 16 kHz、16-bit、单声道 raw PCM。按住 `OK` 键时，每采集 1024 字节（32ms）发送一包。 |
| **文本帧 (按键事件)** | 物理按键事件 | JSON 格式：`{"type":"btn","key":"ok","gesture":"short"}` |
| **文本帧 (录音信令)** | PTT 录音开始/结束标记 | 按住 OK：`{"type":"ptt_start"}`；松开 OK：`{"type":"ptt_end"}` |
| **文本帧 (视口翻页)** | 虚拟视口下一页/上一页请求 | 按 UP/DOWN 键触发：`{"type":"page_req","doc_id":"conv_9821","offset":2}` |

#### 2. 下行数据（服务端 ──► 客户端）
| 帧类型 | 内容与格式 | 客户端处理行为 |
| :--- | :--- | :--- |
| **文本帧 (文字流)** | 实时打字机 Token：<br>`{"type":"delta","text":"字"}` | 追加到当前屏幕 Label，内存恒定仅需局部 Buffer。 |
| **文本帧 (视口切片)** | 虚拟视口分页数据包：<br>`{"type":"page_resp","doc_id":"conv_9821","offset":2,"total":12,"text":"..."}` | 替换当前屏幕显示 Label，内存恒定 < 1KB，零重新握手开销。 |
| **文本帧 (卡片结构体)** | 结构化教育/状态卡片：<br>`{"type":"card_word", "word":"apple", ...}`<br>`{"type":"card_char", "char":"舞", "pinyin":"wǔ", ...}` | 解析后更新 LVGL 卡片布局控件，直接呈现教学信息。 |
| **二进制帧 (音频流)** | MP3 音频切片流（首字节加魔数 `0x02`） | 写入 16KB 环形缓冲区，预充 4KB 后送入 Helix 软解并由 I2S 播放。 |
